...
# speed = 9 #different for each item
...
#     if cy2 > yboundary+20: #different for each item
...
#     cy2 += speed
#     cy3 += speed
#     SetStartLocation(currentStage,CG, 1, cx1, cy1) #different for each level
#     SetStartLocation(currentStage,CG, 0, cx2, cy2) #different for each level
#     SetStartLocation(currentStage,CG, 2, cx3, cy3) #different for each level

#     '''calculate distance, dis = sqrt((cx-newx)**2+(cy-y)**2)'''
#     distance = sqrt((cx2-headX)**2+(cy2-headY)**2) #different for each item
...
if timeIndex==1:    
    bottomBun = burger(9,1,[0,4,3,2,1])
